...
import CalendarService from "./calendar/dataService";
import q from "q";
...
class SpinalBackEnd {
  ...
  async init() {
    try {
      ...
      await Promise.all([this.spatialBack.init(graph), this.viewerBack.init(
        graph), this.calendarService.init(graph)]);
