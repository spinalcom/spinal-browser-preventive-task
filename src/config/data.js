...
let dataService = {
  ...
  async getTickets(rooms, processInfo) {
    ...
    let self = this;
    graph.SpinalGraphService.getChildren(context.info.id).then(
    allProcess => {
        ...
        for (var ticket in floor[level].rooms)
          if (floor[level].rooms[ticket].tickets !== undefined) {
            ...
            for (var el in floor[level].rooms[ticket].tickets)
              if (floor[level].rooms[ticket].tickets[el][
                'processName'] !== undefined)
                if (floor[level]['processNumber'][floor[level].rooms[
                    ticket].tickets[el]['processName']] === undefined)
                  floor[level]['processNumber'][floor[level].rooms[ticket]
                    .tickets[el]['processName']
                  ] = 1;
                else {
                  floor[level]['processNumber'][floor[level].rooms[ticket]
                      .tickets[el]['processName']
                    ] =
                    floor[level]['processNumber'][floor[level].rooms[
                      ticket].tickets[el]['processName']] + 1
                }
          }
  ...
  getAllSteps(allProcess) {
    ...
    for (var process in allProcess) {
      graph.SpinalGraphService.getChildren(allProcess[process].id.get()).then(
        child => {
          for (var nodeId in child) {
            self.StepsNodes[child[nodeId].id.get()] = graph
              .SpinalGraphService.getRealNode(child[nodeId].id.get());
          }
        })
    }
  },
  getTicketsPerRoom(lvl, room_nbr, rooms, processInfo) {
    graph.SpinalGraphService.getChildren(rooms[lvl].rooms[room_nbr].id,
        SPINAL_TICKET_SERVICE_TARGET_RELATION_NAME)
      .then(children => {
        if (children.length > 0) {
          rooms[lvl]['count'] = children.length;
          rooms[lvl].rooms[room_nbr]['tickets'] = [];
          rooms[lvl].rooms[room_nbr]['tickets'] = children.slice(0, children
            .length);
          this.addInfoToTicket(children.slice(0, children.length),
            processInfo);
        }
      })
  },
  addInfoToTicket(tickets, processInfo) {
    ...
    setTimeout(function() {
      for (var ticket in tickets) {
        processe = graph.SpinalGraphService.getRealNode(tickets[ticket]
          .processId.get())

        if (processe !== undefined) {
          if (self.total['count'][processe.info.name.get()] ===
            undefined) {
            self.total['count'][processe.info.name.get()] = 1;
          } else {
            self.total['count'][processe.info.name.get()] = self.total[
              'count'][processe.info.name.get()] + 1;
          }
        }
      }
    }, 2000)
      ...
      for (var floor in floors[index])
        for (var room in floors[index][floor])
          if (typeof floors[index][floor][room].id !== "undefined")
            this.addEquipmentInRoom(floors[index][floor][room].id, floors,
              index, floor, room);

    return Promise.resolve([]);
  },
  addEquipmentInRoom(id, floor, index, floor_lvl, room) {
    graph.SpinalGraphService.getChildren(id, 'hasBIMObject').then(
      equipmments => {
  ...
  getProcessName(obj) {
    ...
    setTimeout(function() {
      for (var node in self.ProcessNodes) {
        ...
        icons[self.ProcessNodes[node].name.get()] = self.ProcessNodes[
          node].icon.get()
      }
    }, 2000);
  },
