...
<template>
  <div class="sidebarProcess">
    ...
    <div v-for="element in elements">
      <v-icon v-on:click="onClick"
              class="icon-display"
              :data="element.data">{{element.icon}}</v-icon>
    </div>
...
export default {
  ...
  methods: {
    update: function(el, parentName) {
      ...
      SpinalServiceTicket.getAllProcessAsync().then(ok =>
        console.log("-------", ok)
      );
    },
    getEvents: function() {
      EventBus.$on("selectCategory", (type, parentName) =>
        this.update(type, parentName)
      );
    },
